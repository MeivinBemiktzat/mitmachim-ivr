...
function createChatFlow(forumCtx) {
  const {
    ...
    // במתמחים טופ בלבד אין אפשרות להשיב בצ'אט בהקלדת טקסט.
  // בכל שאר הפורומים האפשרות נשארת זמינה.
  const allowTypedChatReply = FORUM_SYSTEM_ID !== 'mitmachim';
...
async function sendChatMessage(userCookie, roomId, messageText) {
  const { csrfToken, userCookie: freshCookie } =
    await getCsrfTokenWithCookie(userCookie);

  console.log('[chatFlow] שולח הודעת צ\'אט:', {
    roomId,
  ...
  try {
    response = await http.post(
      `/api/v3/chats/${encodeURIComponent(roomId)}`,
      {
        message: messageText,
        toMid: ''
      },
    ...
    console.error('[chatFlow] שגיאת HTTP בשליחת הודעה:', err.message);
    throw new Error(`שליחת ההודעה נכשלה: ${err.message}`);
  }

  console.log('[chatFlow] תגובת NodeBB לשליחת הודעה:', {
    status: response.status,
    statusText: response.statusText,
    data: response.data
  });

  // כל קוד שאינו 2xx נחשב כישלון.
  if (response.status < 200 || response.status >= 300) {
    const data = response.data || {};

    throw new Error(
      `NodeBB החזיר HTTP ${response.status}: ${
        data?.status?.message ||
        data?.message ||
        data?.error ||
        JSON.stringify(data)
  ...
  // אם NodeBB החזיר במפורש שגיאה בתוך גוף התשובה.
  if (data.error) {
    throw new Error(
      `NodeBB החזיר שגיאה: ${data.error}`
    );
  }
  ...
  if (data.status?.code && data.status.code !== 'ok') {
    throw new Error(
      `NodeBB החזיר שגיאה: ${
        data.status.message || data.status.code
      }`
    );
  }
  ...
  async function recordAndConfirmReply(call) {
    try {
      ...
      await call.id_list_message([
        { type: 'text', data: allowTypedChatReply
  ? 'שירות ההקלטה אינו זמין כרגע, אנא נסו שוב מאוחר יותר או השתמשו בהקלדת טקסט'
  : 'שירות ההקלטה אינו זמין כרגע, אנא נסו שוב מאוחר יותר', removeInvalidChars: true }
  ...
  async function openRoomFlow(call, userCookie, uid, room) {
    ...
    // ואז שליחה בפועל.
    // שלב המענה:
    // במתמחים טופ - הקלטה בלבד.
    // בשאר הפורומים - הקלטה או הקלדת טקסט.
    let replyMethod;
    
    if (allowTypedChatReply) {
      replyMethod = await call.read([
        { type: 'text', data: 'להשבה בהקלטה קולית שתתומלל הקישו 1, להשבה בהקלדת טקסט הקישו 2, לביטול הקישו 0', removeInvalidChars: true }
      ], 'tap', { ...MENU_READ_OPTS, max_digits: 1 });
    } else {
      replyMethod = await call.read([
        { type: 'text', data: 'להשבה בהקלטה קולית שתתומלל הקישו 1, לביטול הקישו 0', removeInvalidChars: true }
      ], 'tap', { ...MENU_READ_OPTS, max_digits: 1 });
    }

    let finalText = null;
    if (replyMethod === '1') {
      finalText = await recordAndConfirmReply(call);
    } else if (replyMethod === '2' && allowTypedChatReply) {
      finalText = await typeAndConfirmReply(call);
    } else {
      return;
    }
