...
router.get('/', async (call) => {
  ...
  // תיקון נוסף: id_list_message הוא directive סופי לתור ה-HTTP הנוכחי -
  // אסור לקרוא ל-call.read מיד אחריו באותו תור (הדבר גרם לשתי תגובות/
  // directives באותה קריאת HTTP וללוג שגיאה מטעה). כאשר בוצעה הכרזה,
  // מסיימים את התור הנוכחי כאן; ימות תפנה שוב לראוטר בבקשת ה-HTTP הבאה,
  // ואז אין יותר התראות "חדשות" לא-מוכרזות ולכן annouceNewNotifications
  // תחזיר false והתפריט הראשי יוקרא כרגיל.
