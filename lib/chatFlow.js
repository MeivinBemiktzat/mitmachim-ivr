 ...
 *
 *   [לא אומת - מבוסס מוסכמת NodeBB read API הכללית "כל דף הופך ל-JSON
 *    ע"י הוספת api/ בתחילת הנתיב", התואמת את שאר הקוד הקיים בפרויקט]:
 *     GET {FORUM_BASE}/api/user/{userslug}/chats/{roomId}
 *       -> מצופה: { messages: [ { content, cleanedContent, fromuid,
 *                    fromUser:{username,displayname,...}, timestamp,
 *                    timestampISO, self, ... }, ... ], roomId, ... }
 *       (מבנה הודעה בודדת מ-payload שדווח בפורומי התמיכה של NodeBB לתגובת
 *       שליחת הודעה - ר' הערה למטה - סביר שדומה למבנה בהיסטוריה)
 *
 *   [לא אומת - מבוסס NodeBB Write API (core, מ-v1.15.0 ואילך) שתומך גם
 *    ב-cookie+CSRF (לא רק Bearer token), באותה שיטה המשמשת כבר את הקוד
 *    הקיים מול /login. הנתיב costume: יש לוודא מול שרת חי]:
 *     POST {FORUM_BASE}/api/v3/rooms/{roomId}
 *       body: { message: "<טקסט ההודעה>" }
 *       headers: { 'x-csrf-token': <מה-config>, Cookie: userCookie }
 *       -> מצופה תגובה בפורמט { status:{code:'ok'}, response:{...} }
 *          (מבנה סטנדרטי של NodeBB Write API v3)
 *
 * אם אחד מהנתיבים הלא-מאומתים לא יעבוד בפועל מול אחד מהפורומים - השגיאה
 * תילכד ותוצג הודעה ידידותית למתקשר (ר' withRetry/try-catch בכל פונקציה
 * למטה), ולא תקריס את השיחה. יש לבדוק בהקדם מול כל 5 הפורומים בפועל
 * ולעדכן את הנתיבים כאן במידת הצורך - שינוי במקום אחד (כאן) משפיע על כל
 * 5 הפורומים בבת אחת.
 *
 * ---------------------------------------------------------------------
...
function createChatFlow(forumCtx) {
   ...
   *  { userCookie, userslug } - זורק שגיאה ברורה בכל שלב (אין הרשמה,
  ...
  async function loginForChats(phone) {
    ...
    // userslug לצורך בניית /api/user/{userslug}/chats/ - נשלף מ-/api/self
    // (נתיב read API סטנדרטי של NodeBB למשתמש המחובר לפי ה-session), כדי
    // לא להסתמך על ניחוש userslug מתוך שם המשתמש (עלול לכלול ניקוד/סלאש
    // שונה מהשם עצמו - NodeBB מנרמל userslug בעצמו).
    let userslug;
    try {
      const { data: self } = await http.get('/api/self', { headers: { Cookie: userCookie } });
      userslug = self?.userslug;
      if (!userslug) throw new Error('לא התקבל userslug מ-/api/self');
    } catch (err) {
      const e = new Error(`SELF_FAILED: ${err.message}`);
      e.code = 'SELF_FAILED';
      throw e;
    }
    return { userCookie, userslug };
  }
  ...
  /** שולף את היסטוריית ההודעות של חדר צ'אט ספציפי. לא אומת מול שרת חי -
   *  ר' תיעוד המודול למעלה. */
  async function fetchChatMessages(userCookie, userslug, roomId) {
    const { data } = await http.get(`/api/user/${encodeURIComponent(userslug)}/chats/${roomId}`, {
      headers: { Cookie: userCookie }
    });
    return data?.messages || [];
  }

  /** שולח הודעת צ'אט חדשה לחדר קיים, דרך NodeBB Write API (v3) עם אימות
   *  cookie+CSRF (אותה שיטה המשמשת את /login הקיים). לא אומת מול שרת חי -
   *  ר' תיעוד המודול למעלה. */
  async function sendChatMessage(userCookie, roomId, messageText) {
    ...
    const { data } = await http.post(`/api/v3/rooms/${roomId}`, { message: messageText }, {
      headers: {
        Cookie: freshCookie,
  ...
  async function openRoomFlow(call, userCookie, userslug, room) {
    let messages;
    try {
      messages = await fetchChatMessages(userCookie, userslug, room.roomId);
    } catch (err) {
  ...
  async function chatsFlow(call) {
    ...
    const { userCookie, userslug } = session;
    ...
    for (;;) {
      ...
      if (key === '7') { if (i > 0) i--; continue; }
      if (key === '5' || key === '') { await openRoomFlow(call, userCookie, userslug, rooms[i]); continue; }
