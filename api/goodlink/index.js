/**
 * מערכת IVR - פורום "גוד לינק" (good-link.org)
 * ================================
 * מערכת טלפונית מלאה לגלישה בפורום good-link.org (מבוסס NodeBB, אותה תשתית
 * פורום בדיוק כמו mitmachim.top/f2.freeivr.co.il/otzaria.org) דרך מערכת
 * "ימות המשיח", מבוססת על ספריית yemot-router2 (מודול API הרשמי).
 *
 * קובץ זה הוא עותק מותאם (מבחינת דומיין בלבד) של api/yemot/index.js
 ...
 * api/goodlink/index.js (בדיוק כמו api/yemot, api/freeivr ו-api/otzaria) -
 * כדי שיהיה אפשר להצביע אליו מהגדרות שלוחה שונה בממשק הניהול של ימות
 * המשיח, ללא שום התנגשות בין המערכות (לא ב-session, לא בקבצי הקלטה, ולא
 * בשלוחות).
 * דומיין הפרויקט (משותף): https://mitmachim-ivr.vercel.app
 * הכתובת הספציפית לשלוחה זו: https://mitmachim-ivr.vercel.app/api/goodlink
 *
 ...
 * ו-api/userStore.js + api/register.js - הערך שנבחר שם הוא "goodlink"),
 ...
 *
 * הערה חשובה על אימות (session): פורום גוד לינק משתמש באותו שם משתמש/סיסמא
 * כמו פורום מתמחים טופ (לפי בקשת המשתמש - אותו חשבון בדיוק בשלושת הפורומים
 * מתמחים טופ/ימות/גוד לינק) - ולכן מודול זה משתמש *באותם* משתני סביבה
 * בדיוק (SERVICE_USERNAME/SERVICE_PASSWORD) כמו api/yemot/index.js ו-
 * api/otzaria/index.js, ואינו מגדיר משתני סביבה נפרדים משלו (בניגוד לדפוס
 * שננקט ב-api/freeivr, ששם יש FREEIVR_SERVICE_USERNAME/
 * FREEIVR_SERVICE_PASSWORD נפרדים - כאן זה מיותר כי מדובר באותו חשבון
 * בפועל). מאותה סיבה, גם משתני הסביבה הקשורים להתראות/לאתר ההרשמה
 * (UPSTASH/CRON/QSTASH) משותפים לכל הפורומים ואינם דורשים הוספה כלשהי.
 *
...
// גוד לינק לבין רשומות שאר הפורומים הנתמכים באותו מספר טלפון (ר' תיעוד
// מפורט ב-userStore.js / getUserCredentials).
const FORUM_SYSTEM_ID = 'goodlink';
// פורום גוד לינק בשורש הדומיין שלו (בדיוק כמו mitmachim.top ו-
// f2.freeivr.co.il, ולא כמו otzaria.org שמותקן תחת תת-נתיב /forum) - לכן
// FORUM_BASE הוא כתובת הדומיין בלבד, ללא תת-נתיב נוסף.
const FORUM_BASE = 'https://good-link.org';
...
// (ר' תיעוד ליד downloadRecording/voiceSearchFlow). לא קשור לפורום good-link.org.
...
const http = axios.create({
  ...
  timeout: 8000,
  headers: {
    'User-Agent': 'MitmachimIVR-GoodLink/1.0 (+https://mitmachim-ivr.vercel.app)',
...
/**
 * ניהול session של "משתמש שירות" קבוע בפורום, לצורך ק��יאה ל-API-ים שדורשים
 ...
 *  https://good-link.org/search?in=titles&term=&matchWords=all&by=&categories=&
...
/** ממיר תגובת /api/search (מבנה posts[].topic ��קונן, ראה תיעוד ליד fetchNewestTopics)
...
//     (מטמון) ולא את התוכ�� העדכני שנכתב זה עתה על גבי אותו path+file_name
...
async function fetchLastPost(tid, slug, postcount) {
  // הערה קריטית (תוקן): לא ניתן לנחש א�� מספר העמוד האחרון לפי postcount בלבד -
...
/* ---------- שלוחה 2: נושאים אחרונים - א��כולות חדשים לפי זמן *יצירת האשכול*
 * (topic.timestamp), תואם לרשימה המוצגת בכתובת:
 * https://good-link.org/search?in=titles&sortBy=topic.timestamp&sortDirection=desc&showAs=topics
...
// תיקיית יעד קבועה לשמ��רת הקלטות החיפוש, כתת-שלוחה של השלוחה הראשית.
...
// הערה קריטית (goodlink): מספר תת-השלוחה כאן (10) שונה בכוונה ממספרי
// תת-השלוחה שנבחרו בגרסאות האחרות (8 עבור מתמחים טופ/אוצריא, 9 עבור
// freeivr) - כדי שאם כמה ממערכות ה-IVR מוגדרות תחת אותה מערכת ימות המשיח
// פיזית (אותו טוקן ניהול/YEMOT_MANAGEMENT_TOKEN), לא תיווצר התנגשות בנתיב
// שמירת קבצי ההקלטה של החיפוש הקולי בין המערכות.
const VOICE_SEARCH_EXTENSION_NUMBER = '10'; // מספר תת-שלוחה קבוע תחת השלוחה הראשית
const VOICE_SEARCH_EXTENSION_TITLE = 'GoodLinkVoiceSearchRecordings'; // שם תיאורי באנגלית - מוצג בממשק הניהול של ימות בלבד, לא חלק מהנתיב
...
const CHAT_RECORD_EXTENSION_NUMBER = '11';
...
router.get('/', async (call) => {
  ...
  for (;;) {
    try {
      const choice = await call.read([
        { type: 'text', data: 'ברוכים הבאים לפורום גוד לינק הקולי', removeInvalidChars: true },
...
app.use((req, res, next) => {
  console.log(`[GOODLINK IN] ${req.method} ${req.originalUrl}`);
  const send = res.send.bind(res);
  res.send = (body) => {
    console.log(`[GOODLINK OUT] ${req.originalUrl} ->`, typeof body === 'string' ? body.slice(0, 300) : body);
    return send(body);
  };
  next();
});
...
// https://<דומיין-הפרויקט>/api/goodlink/
app.use('/api/goodlink', router.asExpressRouter);

if (require.main === module) {
  // פורט מקומי שונה מגרסאות הפורומים האחרות (3000/3002), כדי שניתן יהיה
  // להריץ את כל המערכות בו-זמנית בסביבת פיתוח מקומית ללא התנגשות פורטים.
  const port = process.env.PORT || 3004;
  app.listen(port, () => console.log(`Good-Link IVR פועל על פורט ${port}`));
}
